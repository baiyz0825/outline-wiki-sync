...
func init() {
	dateLogConfig := DateLogConfig{}
	Log = dateLogConfig.NewLogger()
}

func (d *DateLogConfig) NewLogger() *logrus.Logger {
	Logger := logrus.New()
	// 设置是否输出文件名和行号信息
...
func (s *LogFileFormatter) Format(entry *logrus.Entry) ([]byte, error) {
	...
	if entry.Caller != nil {
		...
		// 获取调用日志记录函数的代码行号
		lenCode = entry.Caller.Line
	}
	msg := fmt.Sprintf("[%s] %s [%s:%d] %s\n", strings.ToUpper(entry.Level.String()), timestamp, file, lenCode, entry.Message)
	return []byte(msg), nil
}
...
func (s *LogConsoleFormatter) Format(entry *logrus.Entry) ([]byte, error) {
	...
	_, _ = fmt.Fprintf(b, "\033[3%dm[%s]\033[0m [%s] %s %s\n", color, strings.ToUpper(entry.Level.String()), timestamp, fileVal, entry.Message)
	return b.Bytes(), nil
}
